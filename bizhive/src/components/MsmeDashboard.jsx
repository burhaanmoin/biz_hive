      ...
      <AppBar position="fixed" className="app-bar" sx={{
        ...
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="toolbar-right" style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            <IconButton color="inherit" onClick={handleNotifOpen} sx={{ mr: 1, color: '#222' }}>
              <Badge badgeContent={notifications.length} sx={{
                '& .MuiBadge-badge': { backgroundColor: '#d32f2f', color: '#fff', fontWeight: 700 }
              }}>
                <NotificationIcon sx={{ color: '#222' }} />
              </Badge>
            </IconButton>
            <Menu
            ...
            >
              <Typography variant="subtitle1" sx={{ px: 2, pt: 1, fontWeight: 700, color: '#222' }}>Notifications</Typography>
              {notifications.length === 0 && (
                <MenuItem disabled>No new notifications</MenuItem>
              )}
              {notifications.map((notif, idx) => (
                <MenuItem key={notif.id} sx={{ alignItems: 'flex-start', py: 2, gap: 2 }}>
                  <ListItemAvatar>
                    <Avatar sx={{ bgcolor: notif.type === 'success' ? '#4caf50' : notif.type === 'warning' ? '#ff9800' : '#2196f3', width: 32, height: 32, color: '#222', fontWeight: 700 }}>
                      <NotificationIcon fontSize="small" sx={{ color: '#222' }} />
                    </Avatar>
                  </ListItemAvatar>
                  <ListItemText
                    primary={<span style={{ fontWeight: 600, color: '#222' }}>{notif.title}</span>}
                    secondary={<>
                      <Typography component="span" variant="body2" color="#444">{notif.desc}</Typography>
                      <br />
                      <Typography component="span" variant="caption" color="#666">{notif.time}</Typography>
                    </>}
                  />
                </MenuItem>
              ))}
            ...
            <Button 
              ...
              startIcon={
                <Avatar 
                  ...
                  className="avatar"
                  sx={{ color: '#222', fontWeight: 700, background: '#e3f0ff' }}
                >
                  {enterprise?.enterpriseName ? enterprise.enterpriseName[0] : 'E'}
                </Avatar>
              }
              sx={{ textTransform: 'none', fontWeight: 700, fontSize: '1.1rem', ml: 1, color: '#222' }}
            >
